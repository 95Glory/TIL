...
```
이제 클래스의 설계도의 작성이 끝이 나면 새로운파일에 main메서드를 호출하고 새로운 객체를 new를 통해 생성하고 변수 s에 담아준다 또한 괄호에 기입할 데이터를 적어야 하며 선언된 자료형에 맞게 작성을 해야 한다 예를 들어 int a 를 선언하여 정수형 변수를 만들었는데 "2" 처럼 숫자를 문자로 변경하여 할당하면 오류가 나는 것 처럼 말이다.  
